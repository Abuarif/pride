 ...
 */
        public function inbox() {
                ...
                $this->Message->recursive = 0;
                $status = 0;
                if (isset($this->params['named']['status'])) $status = $this->params['named']['status'] ;
           $criteria['Contact.alias'] = CakeSession::read('Auth.User.username');

                $this->paginate['Message']['conditions']['Contact.alias'] = CakeSession::read('Auth.User.username');
                $this->paginate['Message']['conditions']['Message.status'] = $status;
                $contacts = $this->Message->Contact->find('list');
 ...
 */
        public function outbox() {
                ...
                $criteria['Message.name'] = CakeSession::read('Auth.User.username');
                $contacts = $this->Message->Contact->find('list');
                $messages = $this->paginate($criteria);
                $searchFields = array('contact_id', 'status' => array(
                        'label' => __d('croogo', 'Read'),
