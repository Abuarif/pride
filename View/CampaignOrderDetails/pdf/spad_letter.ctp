...
<body style="font-family:Arial, Helvetica, sans-serif;font-size:20px">
	...
	<div>
		...
		<?php
			$ttlDesign = count($campaignDesigns);
		?>
		
		<?php 
			$counter = 1;
			foreach ($campaignDesigns as $campaignDesign):	
		?>
		
		<b>Visual Iklan <?php echo $counter; ?></b>
		<br /><br />
		...
		<table width="100%" border="1" cellspacing="0" cellpadding="0">
			<tr>
				<td style="padding-top:10px;padding-bottom:10px;text-align:center;">
					<?php	
						echo $this->Html->image(Configure::read('AMS.base_URL').$this->webroot."attachments/".$campaignDesign['CampaignDesign']['files']);
					?>
				</td>
			</tr>
			<tr>
				<td style="text-align:center;height:40px;">
					<i>Gambarajah : Rekabentuk Iklan <?php echo $counter; ?></i>
				</td>
			</tr>
		</table>
			<?php 
				if($counter == $ttlDesign) {
			?>
